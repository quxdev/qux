...
test suite and the ``pagespeed_report`` management command.
        ...
        if os.path.exists(path):
            ...
            for block in FONT_FACE_RE.findall(css):
                if "font-display: block" not in block:
                    findings.append(
                        (icon_font_css, "icon-font", "should use font-display: block")
                    )
    return findings
...
def check_alt_attributes(alt_exceptions=None):
    ...
    for relpath, line_no, match in scan_template_content(IMG_TAG_RE):
        ...
        if "alt=" not in tag_lower:
            missing.append((relpath, line_no, tag))
        elif re.search(r"""alt=["']\s*["']""", tag_lower):
            missing.append((relpath, line_no, tag))
    return missing
...
def check_resource_hints(base_template_relpath, viewport_search_paths):
    ...
    if not re.search(
        r'<link\s+rel="preconnect"\s+href="[^"]*googletagmanager\.com[^"]*"', html
    ):
    ...
    for path in viewport_search_paths:
        full = (
            os.path.join(settings.BASE_DIR, path) if not os.path.isabs(path) else path
        )
...
def check_css_minified(template_relpath):
    ...
    link_re = re.compile(
        r"""<link[^>]+href=(["'])([^"']+\.css)\1""", re.IGNORECASE | re.DOTALL
    )
    ...
    for match in static_re.finditer(content):
        ...
        for prefix_name, prefix_dir in settings.STATICFILES_DIRS:
            candidate = os.path.join(
                prefix_dir, css_path.replace(f"{prefix_name}/", "", 1)
            )
            min_candidate = os.path.join(
                prefix_dir, min_path.replace(f"{prefix_name}/", "", 1)
            )
