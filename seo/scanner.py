...
def check_seo_consistency(seo, path):
    ...
    if canonical and not canonical.endswith(path):
        issues.append(
            ("warning", f"canonical '{canonical}' does not end with '{path}'")
        )
...
def fetch_sitemap_urls(domain, scheme="https"):
    ...
    for url in urls:
        if url.endswith(".xml"):
            try:
                sub_resp = requests.get(url, timeout=REQUEST_TIMEOUT)
...
def scan_site(domain, scheme="https"):
    ...
    try:
        ...
        audit = SEOAudit.objects.create(domain=domain, status="failed")
        return audit
