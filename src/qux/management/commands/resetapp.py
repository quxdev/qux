"""``python manage.py resetapp <app_label> [...] [--wipe | --nuke]`` — clear app data.

Two destructive modes for the listed apps:

- ``--wipe``: ``DELETE FROM`` every model table for each app, then reset
  the auto-increment sequence (via ``qux.utils.mysql.resetsequence``). Schema
  and migrations stay intact; rows are gone.
- ``--nuke``: ``DROP TABLE`` every model table AND ``DELETE FROM
  django_migrations WHERE app = <label>``. Requires interactive
  ``yes`` confirmation. Use to rebuild from a clean migration state in
  a dev DB. The ``users`` pseudo-app label means "delete all rows from
  the configured AUTH_USER_MODEL." Drops nothing else for that label.

App labels are validated against ``^[a-zA-Z][a-zA-Z0-9_]*$`` and against
``apps.get_app_config()`` before any destructive operation runs. Production
use is risky by design — favor it for local dev resets, not shared environments.

Without ``--wipe`` or ``--nuke`` the command is a no-op (good for dry-run
sanity-checking the app-label list).
"""
    ...
    def handle(self, *app_labels, **options):
        ...
        if bad_app_labels:
            for app in bad_app_labels:
                self.stderr.write(f"App '{app}' could not be found. Is it in INSTALLED_APPS?")
            sys.exit(2)

        if self.nuke:
            confirm = input(f"This will DROP tables for {self.app_labels}. Type 'yes' to confirm: ")
            if confirm != "yes":
