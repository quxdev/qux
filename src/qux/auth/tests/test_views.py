...
class TestSendSignupVerificationEmail(TestCase):
    ...
    @patch("qux.auth.views.appviews.render_to_string", return_value="<html>activate</html>")
    @patch("qux.auth.views.appviews.send_mail")
    def test_sends_email(self, mock_send_mail, _mock_render_to_string):
        request = self.factory.get("/auth/signup/")
        _add_middleware(request)
        ...
        assert kwargs["recipient_list"] == ["emailuser@qux.dev"]
        assert kwargs["subject"] == "Activate your account."
        assert kwargs["html_message"] == "<html>activate</html>"
...
class TestLoginRequest(TestCase):
    ...
    def test_post_form_valid_but_authenticate_none(self, _mock_auth, _mock_render):
        """Form.is_valid() True but authenticate returns None."""
        request = self.factory.post(
            "/auth/login/",
            {
                "username": "lruser",
