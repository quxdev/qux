...
class BaseLoggingMixin:
    ...
    def finalize_response(self, request, response, *args, **kwargs):
        ...
        if should_log:
            ...
            # The logging path must NEVER fail an API request. Any exception
            # from handle_log (DB down, serializer error, full disk, broken
            # downstream handler) gets logged and swallowed so the response
            # to the user is unaffected. Broad except is the correct shape;
            # the alternative is letting the log subsystem take down the API.
            try:
                self.handle_log()
            except Exception:
    ...
    def _get_user(request):
        """Get user."""
        user = request.user
        default_userid = f"userid_{user.id}"
