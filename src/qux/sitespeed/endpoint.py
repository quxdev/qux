...
the ``sitespeed`` management command.
...
class Endpoint:
    ...
    def render_compact(self, include_css=False, include_cache=False):
        """Return a compact summary of endpoint metrics as a multi-line string.

        Returns the rendered text; the caller writes it (Django management
        command via ``self.stdout.write``).
        """
        ...
        if include_css:
            ...
            cell_c = cell(f"BlockingJS={fmt_int(self.data.get('blocking_script_count'))}")
    ...
    def render_warnings(self, label):
        """Return warnings as a multi-line string, or empty string if none.

        Caller checks truthiness to decide whether anything was rendered.
        """
    ...
    def _collect(
        ...
        try:
            with httpx.Client(http2=True, timeout=self.timeout) as client:
                ...
                compressed, uncompressed, ratio = self._measure_compression(client, self.url)
    ...
    def _parse_html_metrics(html, data):
        ...
        data["blocking_script_count"] = len(re.findall(r"<script(?![^>]+defer)[^>]*>", html, re.I))
            ...
            with (
                ...
                if cert:
                    ...
                    if not_after:
                        ...
                        data["cert_expiry_days"] = int((expiry_ts - time.time()) / 86400)
                    ...
                    data["cert_san"] = [value for type_, value in sans if type_ == "DNS"]
    ...
    def _generate_warnings(self, hostname, data):
        ...
        if data["dns_resolve_ms"] is not None and data["dns_resolve_ms"] > 100:
            data["warnings"].append(f"Slow DNS resolution ({int(data['dns_resolve_ms'])}ms)")
    ...
    def _warn_security_headers(data):
        ...
        for header_name in SECURITY_HEADERS:
            ...
            if value is None:
                ...
                data["warnings"].append(f"X-Content-Type-Options is '{value}', expected 'nosniff'")
    ...
    def _warn_certificate(hostname, data):
        ...
        if data["cert_expiry_days"] is not None and data["cert_expiry_days"] < 14:
            data["warnings"].append(f"TLS certificate expires in {data['cert_expiry_days']} days")

        if data["cert_san"] and hostname not in data["cert_san"]:
            matched = any(
                san.startswith("*.") and hostname.endswith(san[1:]) for san in data["cert_san"]
            )
            if not matched:
    ...
    def _follow_redirects(client, url, data):
        ...
        for _ in range(max_hops):
            ...
            if response.is_redirect:
                data["redirect_chain"].append({"url": current_url, "status": response.status_code})
    ...
    def _resolve_dns(hostname, data):
        ...
        try:
            results_v6 = socket.getaddrinfo(hostname, 443, socket.AF_INET6, socket.SOCK_STREAM)
            if results_v6:
