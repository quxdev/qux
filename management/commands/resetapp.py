...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            '--wipe', dest='wipe', action='store_true', default=False,
            help='Delete all data'
        )
        parser.add_argument(
            '--nuke', dest='nuke', action='store_true', default=False,
    ...
    def handle(self, *app_labels, **options):
        ...
        if bad_app_labels:
            for app in bad_app_labels:
                self.stderr.write("App '%s' could not be found. Is it in INSTALLED_APPS?" % app)
            sys.exit(2)
