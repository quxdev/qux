...
class TestTagMethods(TestCase):
    ...
    def test_gettaglist_removes_empty(self):
        """Cover line 227: gettaglist removes empty string after join/split."""
...
class TestPostSaveAuditSignal(TestCase):
    def test_audit_trail_created_on_change(self):
        ...
        details = TestAuditDetails.objects.filter(
            audit_summary__in=summaries, field_name="name"
        )
...
class TestQuxModelToDictM2MAttributeError(TestCase):
    """Cover line 58: M2M AttributeError branch when values_list is not available."""
...
class TestQuxModelToDictExcludeNone(TestCase):
    """Cover line 64: exclude_none=True skips None values."""
...
class TestRandomizeEmailAndURLField(TestCase):
    """Cover lines 153, 155: EmailField/URLField randomize branches.
...
class TestRandomizeUnknownFieldType(TestCase):
    """Cover lines 162-163: else branch in randomize for unknown field types."""
...
class TestGettagsRemovesEmpty(TestCase):
    """Cover line 227: gettags removes empty string from tags list."""
...
class TestPreSaveSlugAlreadySet(TestCase):
    """Cover line 242: pre_save_coremodel returns early when slug is already set."""
...
class TestPostInitAuditModeStoresOld(TestCase):
    """Cover line 264: post_init_coremodel stores __old when AUDIT_MODE=True."""
...
class TestPostSaveAuditModeDisabled(TestCase):
    """Cover line 276: post_save_coremodel returns early when AUDIT_MODE=False."""
...
class TestPostSaveAuditSummaryNone(TestCase):
    """Cover line 292: audit_summary or audit_details is None."""
...
class TestPostSaveAuditUserNone(TestCase):
    """Cover lines 296, 299-303: user is None, tries _user, still None, skips audit."""
...
class TestPostSaveAuditFileField(TestCase):
    """Cover lines 313-314: FileField handling in audit diff."""
...
class TestPostSaveAuditDateField(TestCase):
    """Cover lines 317-320: DateField/DateTimeField handling in audit diff."""
    ...
    def test_audit_datetime_field_change(self):
        ...
        obj = AuditedFileItem.objects.create(
            name="dttest",
            event_datetime=datetime.datetime(2024, 1, 1, 12, 0, 0),
            user=user,
        )
        obj = AuditedFileItem.objects.get(pk=obj.pk)
        obj.event_datetime = datetime.datetime(2025, 6, 15, 14, 30, 0)
        obj.save()
...
class TestQuxModelToDictFieldDoesNotExist(TestCase):
    """Cover lines 59-60: FieldDoesNotExist branch in M2M handling."""
...
class TestSlugCollisionWhileLoop(TestCase):
    """Cover line 255: while loop for slug collision in pre_save."""
...
class TestRandomizeDebugLog(TestCase):
    """Cover line 131: logger.debug in randomize when DEBUG=True."""
...
class TestDebugVerbosePreSave(TestCase):
    """Cover line 242: pre_save_coremodel DEBUG_VERBOSE logging."""
...
class TestDebugVerbosePostInit(TestCase):
    """Cover line 264: post_init_coremodel DEBUG_VERBOSE logging."""
...
class TestDebugVerbosePostSave(TestCase):
    """Cover line 278: post_save_coremodel DEBUG_VERBOSE logging."""
