...
from qux.utils.phone import fakephonenumber, format_phone_number, phone_number
...
class TestFakePhoneNumber(SimpleTestCase):
    def test_indian_number(self):
        result = fakephonenumber("IN")
        assert result is not None
        assert result.startswith("+91")

    def test_non_indian_returns_none(self):
        result = fakephonenumber("XX")
        assert result is None
...
class TestFormatPhoneNumber(SimpleTestCase):
    ...
    def test_international_format_non_in_non_us(self):
        """Cover line 79: non-IN, non-US number in national format falls to international."""
...
class TestPhoneNumberPlusPrefixBranch(SimpleTestCase):
    """Cover line 32: phone_number parsing with '+' prefix succeeds."""
...
class TestPhoneNumberElseBranch(SimpleTestCase):
    """Cover lines 35-39: NumberParseException and else branches."""
